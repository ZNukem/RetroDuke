...
#define HEAD   "RetroDuke DN3D Unregistered Shareware v"VERSION" "
#ifdef PLUTOPAK
#define HEAD2  "RetroDuke DN3D v"VERSION" - Atomic Edition"
#else
#define HEAD2  "RetroDuke DN3D Full Version v"VERSION
#endif
#define HEADA  "RetroDuke DN3D AUSSIE Unregistered Shareware v"VERSION
#define HEAD2A "RetroDuke DN3D AUSSIE Full Version v"VERSION
...
#ifdef VOLUMEALL
    #ifdef AUSTRALIA
        printstr(40-(strlen(HEAD2A)>>1),0,HEAD2A,55);
    #else
        printstr(40-(strlen(HEAD2)>>1),0,HEAD2,55);
    #endif
#else
    #ifdef AUSTRALIA
        printstr(40-(strlen(HEADA)>>1),0,HEADA,55);
    #else
        printstr(40-(strlen(HEAD)>>1),0,HEAD,55);
    #endif
#endif

#ifdef BETA
    printstr(0,1,"BETA VERSION BETA VERSION BETA VERSION BETA VERSION BETA VERSION BETA VERSION ",55);
#endif

#ifdef ONELEVELDEMO
    printstr(33,1,"ONE LEVEL DEMO",55);
#endif

    ud.multimode = 1;
    printstr(0,1,"               ZNukem - read docs for more info (Build: 06-22-23)               ",55);

//    printstr(0,2,"  ***     DUKE NUKEM v1.4 BETA VERSION.  USED FOR INTERNAL USE ONLY!!!     ***  ",55);
